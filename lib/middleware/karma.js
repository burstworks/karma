...
var SCRIPT_TAG = '<script type="%s%s" src="%s"></script>'
...
var JAVASCRIPT_VERSION = ';version=%s'
...
var createKarmaMiddleware = function (filesPromise, serveStaticFile,
  ...
  /* config.client */ client, /* config.scriptsWithVersion */ scriptsWithVersion) {
      ...
      return filesPromise.then(function (files) {
        serveStaticFile(requestUrl, response, function (data) {
          ...
          var scriptTags = files.included.map(function (file) {
            ...
            if (scriptsWithVersion !== undefined) {
              for (var index=0; index<scriptsWithVersion.length; index++){
                var definition = scriptsWithVersion[index];
                if (filePath.indexOf(definition.path) > -1){
                   return util.format(SCRIPT_TAG, SCRIPT_TYPE[fileExt] || 'text/javascript', 
                                   util.format(JAVASCRIPT_VERSION, definition.version || '1.7'), filePath) 
                }
              }
            }
...
createKarmaMiddleware.$inject = ['filesPromise', 'serveStaticFile',
  'config.basePath', 'config.urlRoot', 'config.client', 'config.scriptsWithVersion']
